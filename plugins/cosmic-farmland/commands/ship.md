...
6a. **Verify production deploy (skip if repo has no `railway.json`).** A green PR check is not the same as a successful prod deploy. PR-preview and prod can pass different gates: preview-only test DB, env-conditional pre-deploy commands, missing prod secrets. Real incident 2026-05-01: PR #421 merged green, but prod deploys had been silently failing pre-deploy on every push since #419 (a guard in src/test/setup.ts rejected mainline DB; PR-preview used switchyard so the gap was invisible). Surfaced only when a user asked about an unrelated service. Do not let `/ship` return success while prod is stale.
   ...
   **Diff-gate skip (fail-safe: verify unless provably inert).** Prod-verify exists to catch failures in build, pre-deploy, or boot. Skipping is an optimization, and the asymmetry matters: a false-skip silently ships broken prod (the exact incident this gate was born from), while a false-verify just burns a few minutes polling. So verify by DEFAULT and skip ONLY when the merged diff is provably non-deploy-affecting -- pure docs/markdown/test/CI-metadata. Do NOT enumerate deploy-affecting paths per repo: that allowlist is repo-specific and goes stale (incident 2026-05-27: a hardcoded `src/server/`-style allowlist false-skipped a lenny-explorer ship whose frontend ships *inside* the Docker image, so a `src/components/` UI diff matched nothing and prod-verify never ran). Before polling, check the merged diff:
   ```bash
   gh pr diff <pr> --name-only | grep -qvE '\.mdx?$|^docs/|\.test\.|\.spec\.|^\.github/' && NEEDS_VERIFY=1 || NEEDS_VERIFY=0
   ```
   `grep -qv` succeeds if ANY changed file falls outside the inert set, flipping `NEEDS_VERIFY=1`. If `NEEDS_VERIFY=0` (pure docs/markdown/test/CI changes), skip the poll entirely and note "prod-verify skipped (diff is docs/test-only)" in the final report.
