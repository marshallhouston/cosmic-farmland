...
6a. **Verify prod deploy** (skip if no Railway config: `railway.json`/`.toml` or `.railway/railway.{ts,py,go}`). Run `${CLAUDE_PLUGIN_ROOT}/scripts/ship-verify-deploy.sh <pr> <service> [health-url] [environment]` with `run_in_background: true`; report the deploy line as a follow-up when it exits. The script handles the timeout/auth/diff-gate/poll logic and exits 0 (verified or cleanly skipped) or 2 (couldn't verify → tell user to check the dashboard). Get `<service>` from `railway status --json` (the service whose source repo matches this one); derive `[health-url]` from the prod domain + the repo's health path (e.g. `https://<domain>/api/health`) — omit it if unknown. `[environment]` defaults to `production`; pass it only if the repo's prod env is named otherwise. A green check is not a green deploy — preview and prod pass different gates. **If the deploy line reports `FAILED`/`CRASHED`, investigate per the step-3a discipline (pull logs, name root cause) — don't just relay the string.**

6b. **Prune the PR's preview env** (skip if the repo has no `railway:prune-envs` script in `package.json`). A merged PR's Railway preview environment goes stale the instant it merges, but Railway's auto-delete-on-close leaks stragglers. They pile up silently until the CLI's project query decode-errors (`expected value at line 1 column 1`), which breaks `railway status`, `railway link`, and the step-6a auth probe. Delete it at the source: `BUN_OPTIONS= bun run railway:prune-envs --pr <pr>` (foreground, fast). The `BUN_OPTIONS=` prefix is load-bearing under Claude Code: a session-injected `BUN_OPTIONS=--preload ...` (lapdog/claude_intercept) prepends `--preload` before the `run` subcommand, which makes `bun run <script>` bail to its help text and silently no-op the prune. Clearing it for this one command restores `bun run`; direct `bun <file>` invocations are unaffected. (why: 2026-06-13 -- prune silently failed every ship under Claude.) The repo's script must scope deletion to that single PR's env and refuse open PRs. Generic across repos -- gated on the script existing, so repos without it no-op. Incident 2026-06-05: 17 stragglers piled to 21 and broke every env-scoped CLI op in preach-hub.

6c. **Run the deploy-gap runbook** (skip if the PR body has no `Post-merge:` block). Some merges do not apply themselves: a seed script, a migration, an env var, an external config push. CI is green, the deploy is green, and prod still diverges from what the code expects, silently, because the step lived in a human's head. So a PR that needs one declares it in its body as a fenced block introduced by `Post-merge:`, and `/ship` executes that block after the deploy verifies, then reports what it ran and what came back. Two rules: the block must be idempotent (a re-ship must not double-apply), and it must end in a command that *checks* rather than asserts - a verification whose non-zero exit means the step did not take. Do not report "shipped" while a declared runbook is unrun or its verification failed; name the command that failed and stop. If the PR carries a `risk:yellow` deploy-gap reason but no `Post-merge:` block, that is the gap - flag it rather than guessing the commands. (why: preach-hub card-authoring PRs were all risk:yellow on the deploy-gap modifier because the prod seed was manual; nothing checked the seed ran, so "merged" and "live" quietly meant different things.)

7. **Report.** One or two sentences: PR number, merged URL, cleanup status. Name the runbook result if step 6c ran. Note if prod-verify is running in background (status follows) or was skipped. Nothing more.
