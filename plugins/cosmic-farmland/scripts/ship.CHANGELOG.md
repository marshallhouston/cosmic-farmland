...
- **PR #29 (2026-06-17)** -- diff-gate read an EMPTY `gh pr diff --name-only` as "docs/test-only" and skipped prod-verify with exit 0; this step runs right after merge when the just-merged, branch-deleted PR briefly returns no files, so a real frontend diff (Analyze.tsx + seedModel.ts) got no health check. Fix: capture the list and guard `[ -n "$files" ]` before the inert-grep so an empty list falls through to VERIFY, matching the poll loop's existing empty-tolerance.
